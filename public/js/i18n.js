  ...
  class I18n {
    ...
    applyTranslations() {
      ...
      document.querySelectorAll("[data-i18n-links]").forEach((element) => {
        ...
        try {
          const links = JSON.parse(
            element.getAttribute("data-i18n-links-data") || "[]",
          );
      ...
      document.querySelectorAll(".lang-menu-item").forEach((el) => {
        el.classList.toggle(
          "selected",
          el.getAttribute("data-code") === this.currentLang,
        );
      });
    ...
    setupDropdown(triggerId, menuId) {
      ...
      SUPPORTED_LOCALES.forEach((locale) => {
        ...
        btn.className =
          "lang-menu-item" +
          (locale.code === this.currentLang ? " selected" : "");
    ...
    setupLanguageSelector() {
      if (!this.configLoaded || SUPPORTED_LOCALES.length === 0) {
        console.error(
          "Cannot setup language dropdowns: configuration not loaded",
        );
        return;
      }
