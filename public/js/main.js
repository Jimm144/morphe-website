...
document.addEventListener("DOMContentLoaded", () => {
      ...
      if (val) {
        ...
        if (key === "faq.a5") {
          ...
          el.innerHTML = val
            ...
            .replace(
              "%2",
              '<a href="https://morphe.software/donate">donations</a>',
            );
        } else {
          el.textContent = val;
        }
  ...
  function setupDropdown(triggerId, menuId) {
    ...
    langs.forEach((lang) => {
      ...
      let iconName = "check";
      btn.innerHTML =
        '<span class="material-symbols-rounded check-mark" style="-webkit-mask: url(icons/' + iconName + '.svg) no-repeat center; mask: url(icons/' + iconName + '.svg) no-repeat center; -webkit-mask-size: contain; mask-size: contain; background-color: currentColor;"></span><span class="lang-name">' +
        lang.name +
        "</span>";
    ...
    () => {
      if (!isScrolling) {
        window.requestAnimationFrame(() => {
          ...
          document
            .querySelectorAll(".nav-link, .drawer-link")
            .forEach((link) => {
  ...
  if (appTabsContainer)
    appTabsContainer.insertBefore(tabInd, appTabsContainer.firstChild);
  ...
  if (featWrap) {
    ...
    featWrap.addEventListener("touchend", (e) => {
      ...
      if (Math.abs(diffX) > 50) {
        if (diffX > 0 && activeIndex < tabBtns.length - 1)
