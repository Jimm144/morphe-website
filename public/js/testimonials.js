...
(function () {
  ...
  function createTestimonialCard(testimonial) {
    const avatar = testimonial.author
  ...
  function loadTestimonials() {
    ...
    return reorderByIndexes(
      [2, 9, 3, 16, 4, 18, 5, 10, 6, 11, 7, 13, 12, 15, 17, 14, 19, 8].map(
        (n) => n - 1,
      ),
      testimonials,
    );
  ...
  function destroyCarousel() {
    ...
    if (track) {
      ...
      track
        .querySelectorAll(".testimonial-card-clone")
  ...
  function initializeCarousel() {
    ...
    function measure() {
      ...
      const carouselPaddingL =
        parseFloat(getComputedStyle(carousel).paddingLeft) || 0;
      const carouselPaddingR =
        parseFloat(getComputedStyle(carousel).paddingRight) || 0;
      // Available width after carousel's own padding (peek gaps)
      const carouselW =
        carousel.clientWidth - carouselPaddingL - carouselPaddingR;
  ...
  function init() {
    ...
    if (
      ...
      window.i18n.translations.testimonials
    ) {
      const testimonials = loadTestimonials();
      renderTestimonials(testimonials);
      setTimeout(() => {
